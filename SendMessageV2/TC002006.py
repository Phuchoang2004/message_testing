...
class TC002006(unittest.TestCase):
    def setUp(self):
    ...
    def test_t_c002006(self):
        ...
        driver.find_element_by_css_selector("textarea[data-region='send-message-txt']").send_keys("Hello, how are you?")
        driver.execute_script("arguments[0].click();", driver.find_element_by_css_selector("button[data-action='send-message']"))
        time.sleep(5)
        try: self.assertRegex(driver.find_element_by_xpath("(//div[@data-region='message' and contains(@class,' send ')])[last()]//p").text, r"^Hello, how are you[\s\S]$")
