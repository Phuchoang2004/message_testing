...
class TC002002(unittest.TestCase):
    def setUp(self):
    ...
    def test_t_c002002(self):
        ...
        driver.find_element_by_css_selector("textarea[data-region='send-message-txt']").send_keys("a")
        driver.execute_script("arguments[0].click();", driver.find_element_by_css_selector("button[data-action='send-message']"))
        time.sleep(5)
        try: self.assertEqual("a", driver.find_element_by_xpath("(//div[@data-region='message' and contains(@class,' send ')])[last()]//p").text)
