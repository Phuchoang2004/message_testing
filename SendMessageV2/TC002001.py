...
class TC002001(unittest.TestCase):
    def setUp(self):
    ...
    def test_t_c002001(self):
        ...
        driver.find_element_by_xpath("//div[@id='view-overview-messages-toggle']/button/span/span/i").click()
        driver.find_element_by_css_selector("a[data-conversation-id='4']").click()
        driver.execute_script("arguments[0].click();", driver.find_element_by_css_selector("button[data-action='send-message']"))
        time.sleep(5)
        try: self.assertEqual("", driver.find_element_by_css_selector("textarea[data-region='send-message-txt']").get_attribute("value"))
