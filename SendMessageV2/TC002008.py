...
class TC002008(unittest.TestCase):
    def setUp(self):
    ...
    def test_t_c002008(self):
        ...
        driver.execute_script("var el=arguments[0]; el.value=arguments[1]; el.dispatchEvent(new Event('input', {bubbles:true}));", driver.find_element_by_css_selector("textarea[data-region='send-message-txt']"), u"Hi there! 😊🎉")
        driver.execute_script("arguments[0].click();", driver.find_element_by_css_selector("button[data-action='send-message']"))
        time.sleep(5)
        try: self.assertEqual(u"Hi there! 😊🎉", driver.find_element_by_xpath("(//div[@data-region='message' and contains(@class,' send ')])[last()]//p").text)
